...
class ShenlanEncode(object):
    ...
    def sencode(self, msg, key):
        ...
        for i in range(0, l, 4):
            pwd.append(
                self.ordat(msg, i) | self.ordat(
                    msg, i + 1) << 8 | self.ordat(msg, i + 2) << 16
    ...
    def js_base64(self, s):
        ...
        if len(s) - imax == 1:
            b10 = self._getbyte(s, i) << 16
            x.append(self._ALPHA[(
                b10 >> 18)] + self._ALPHA[((b10 >> 12) & 63)] + self._PADCHAR + self._PADCHAR)
        elif len(s) - imax == 2:
...
class NwafuLogin(object):
    ...
    def get_ip(self):  # 获取 ip 地址
        get_ip_request = requests.get(
            self.host_login_page_url, headers=self.default_headers)
        # print(get_ip_request.text)
        ip_address = re.findall(
            '        ip     : "(.*?)",', get_ip_request.text, re.S)[0]
        if ip_address:
            print(f"Your IP address: <{ip_address}>")
            return ip_address
        else:
            print('Failed to get your IP address, please check your connection')
            return None

    def get_challenge(self):  # 获取 Token
        get_challenge_params = {
            "callback": 'jQuery11277455887669735664_' + str(int(time.time() * 1000)),
            "username": self.username,
            "ip": self.ip,
            '_': str(int(time.time() * 1000))
        }
        get_challenge_request = requests.get(self.get_challenge_url, headers=self.default_headers, params=get_challenge_params)
        challenge = re.search('"challenge":"(.*?)"', get_challenge_request.text).group(1)
        if challenge:
            return challenge
        else:
            print('Failed to fetch token, please check your connection')
            return None

    def gen_sent_keys(self, challenge, info_dict):  # 生成需要的密钥并发送至服务器
        item = ShenlanEncode(challenge=challenge, info_dict=info_dict)
        login_info_params = {
            'callback': 'jQuery1124064',  
            'action': 'login',
            'username': self.username,
            'password': item.md5,
            'ac_id': item.ac_id,
        ...
        log_in_response = requests.get(self.log_in_url, headers=self.new_headers, params=login_info_params)
        log_in_result = re.findall(
            '"res":"(.*?)"', log_in_response.text, re.S)[0]
        if log_in_result == 'ok':
            self.get_login_info()
        else:
            print('{:-^41}'.format('Failed to authenticate'))
    ...
    def get_login_info(self):  # 获取表示登录状态的代码
        ...
        response = requests.get(self.get_login_info_url,
        ...
        if error_info == 'ok':
            ...
            user_name = re.search(r'"user_name":"(([a-zA-Z]|[0-9])*)"',str_login_info).group(1)

            user_balance = re.search(
                r'"user_balance":(.*?),', str_login_info).group(1)
            sum_bytes = re.search(r'"sum_bytes":(\d+),',str_login_info).group(1)
            print('\n{:-^41}'.format('Login successfully'))
            print('{: ^20}'.format('User name') +
                  '-' + '{: ^20}'.format(user_name))
            print('{: ^20}'.format('Balance') + '-' +
                  '{: ^20}'.format(user_balance))
            print('{: ^20}'.format('Used MB') + '-' +
                  '{: ^20.2f}'.format(int(sum_bytes)/1000000))
            print('{:-^41}\n'.format(''))
        else:
            print('Failed to get authentication status: ' + f'<{error_info}>')
